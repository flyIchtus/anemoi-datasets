...
import anemoi.datasets.create.sources as anemoi_srcs
...
class Accumulation:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        source: str
            Name of the source accumulated (mars, grib, etc...)
        param : str
            Parameter name.
        date : int
    ...
    def check(self, field: Any) -> None:
        ...
        if self._check is None:
            self._check = field.metadata(namespace=self.source)
        ...
        source_metadata = field.metadata(namespace=self.source)
        keys1 = sorted(self._check.keys())
        keys2 = sorted(source_metadata.keys())

        assert keys1 == keys2, (keys1, keys2)

        for k in keys1:
            if k not in ("step",):
                assert self._check[k] == source_metadata[k], (k, self._check[k], source_metadata[k])
    ...
    def date_time_steps(
        ...
        *,
        ...
        """Generates date-time steps, formatted for the specific source.
        ...
        dates : List[datetime.datetime]
            List of dates.
        step1 : int
            First step of the forecast from which to compute accumulation
        step2 : int
            Second step of the forecast (end of accumulation period)
        source: Any,
            The source module 
        frequency : Optional[int]
            Frequency of accumulation.
        ...
        -------
        Generator[Tuple, None, None]
            A generator of date-time steps, formatted for the spefic source of the accumulation.
        """
        ...
        for valid_date in dates:
            ...
            if base_date.hour not in base_times:
                ...
                # this will adjust the base_date hour to an hour where there is a forecast starting
                # (towards the past)
                # the add_step is adjusted accordingly to account for the longer period
                while base_date.hour not in base_times:
                    base_date -= datetime.timedelta(hours=1)
                    add_step += 1

            yield cls._date_time_step(
                source=source,
                base_date=base_date,
                step1=step1,
                step2=step2,
    ...
    def _date_time_step(
        ...
        *,
        ...
        """Generates a date-time step, formatted to the accumulation source.
        This method is abstract and left to sub-classes.

        Parameters
        ----------
        source: Any
            The source module in sources package, corresponding to original data source
        base_date : datetime.datetime
            The base date.
        step1 : int
            First step.
        ...
        -------
        Tuple
            A tuple representing the date-time step.
        """
        pass
...
class AccumulationFromStart(Accumulation):
    ...
    def adjust_steps(self, startStep: int, endStep: int) -> Tuple[int, int]:
        """Adjusts the start and end steps.
           Check that start and end coincide (end of accumulation)
    ...
    def _date_time_step(
        ...
        *,
        ...
        ----------
        source: Any
            The source module in sources package
        base_date : datetime.datetime
            The date at which accumulation starts.
        step1 : int
            First step.
        step2 : int
            Second step.
        ...
        return source.format_date_for_source(base_date, steps)
...
class AccumulationFromLastStep(Accumulation):
    ...
    def _date_time_step(
        ...
        *,
        ...
        return source.format_date_for_source(base_date,tuple(steps))
...
class AccumulationFromLastReset(Accumulation):
    ...
    def _date_time_step(
        ...
        *,
        ...
        return source.format_date_from_source(base_date, tuple(steps))
...
def _compute_accumulations(
    ...
    **kwargs
    ...
    kwargs:
        Extra args to build the accumulation datasource from original data source
    Returns
    -------
    Any
    ...
    # specifying the origin data source
    source = getattr(anemoi_srcs,request.get("source","mars"))
    
    date_time_steps = AccumulationClass.date_time_steps(
        dates=dates,
        step1=step1,
        step2=step2,
        source=source,
    ...
    for date, time, steps in date_time_steps:
    ...
    ds = source.load_for_accumulation(
        context, dates, *requests, **kwargs
    )

    accumulations = {}
    for a in [
        AccumulationClass(out, source=source, frequency=frequency, accumulations_reset_frequency=accumulations_reset_frequency, **r)
        for r in requests
    ]:
        for s in a.steps:
...
def accumulations(
    ...
    **request : Any
    ...
    if (
        ...
        LOG.warning(
            "🧪️ Experimental features: Using accumulations2 (mars data), because class=ea stream=oper and accumulation_period=24"
        )
    ...
    # of the forecast.
    # This retrieves the 'data_accumulation_period' if specified
    kwargs = source.format_accumulation_kwargs(request)
